...
import { 
    ...
    Link as LinkIcon, Play, Shield, Terminal, Zap, FileCheck, CheckCircle, AlertCircle, Download, FileText, Activity
} from 'lucide-react';
import { 
...
import { analyzeSystemRisk, generateAuditAttack, simulateTargetResponse, judgeInteraction, generateBenchmarkPrompt } from '../services/geminiService';
...
const NewAssessment: React.FC<NewAssessmentProps> = ({ onProjectCreate, currentUser }) => {
  ...
  // Technical State
  // Removed connectionType state as we now strictly enforce API
  ...
  const handleTestConnection = async () => {
    ...
    setTimeout(() => {
        ...
        // If URL looks somewhat valid (basic check)
  ...
  const runSingleTest = async (protocol: TestProtocol) => {
    ...
    const response = await simulateTargetResponse(prompt);
  ...
  const getProgress = (category: TestCategory) => {
      const relevant = protocols.filter(p => p.category === category && displayTier && p.requiredForTiers.includes(displayTier));
      if (relevant.length === 0) return 100; // Not applicable
      const completed = relevant.filter(p => testResults.find(r => r.protocolId === p.id && r.status !== 'RUNNING' && r.status !== 'PENDING'));
      return (completed.length / relevant.length) * 100;
  };
  ...
  const generateSafetyReportPDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("2. Benchmark Performance", 14, finalY);

    const suiteAProgress = getProgress('SUITE_A_CORE');
    const suiteBProgress = getProgress('SUITE_B_ADVERSARIAL');

    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Core Safety Compliance (Suite A)", 14, finalY + 10);
    doc.setFillColor(200, 200, 200);
    doc.rect(14, finalY + 12, 100, 6, 'F'); // Background bar
    doc.setFillColor(0, 158, 219);
    doc.rect(14, finalY + 12, suiteAProgress, 6, 'F'); // Progress bar
    doc.text(`${suiteAProgress.toFixed(0)}%`, 120, finalY + 16);

    doc.text("Adversarial Resilience (Suite B)", 14, finalY + 25);
    doc.setFillColor(200, 200, 200);
    doc.rect(14, finalY + 27, 100, 6, 'F');
    doc.setFillColor(220, 50, 50); // Red/Orange for adversarial
    doc.rect(14, finalY + 27, suiteBProgress, 6, 'F');
    doc.text(`${suiteBProgress.toFixed(0)}%`, 120, finalY + 31);
    ...
    const logData = logs.map(l => {
        return [
            ...
            l.role,
            l.content.length > 100 ? l.content.substring(0, 100) + '...' : l.content,
            l.metadata?.verdict || '-'
        ];
    });
                ...
                <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg mb-6 flex items-start space-x-3">
                        ...
                        <div className="text-sm text-blue-800">
                            ...
                            File uploads are disabled for security in the sandbox environment. Please provide a live inference endpoint for the Evaluator Agent to audit.
                        </div>
                    </div>
                        ...
                        >
                            {isTestingConnection ? (
                                ...
                                <>
                                    ...
                                    <span>{connectionStatus === 'SUCCESS' ? 'Connection Verified' : connectionStatus === 'ERROR' ? 'Connection Failed' : 'Test Connection'}</span>
                                </>
                            )}
                        </button>
                    </div>
                ...
                <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                     ...
                     <div className="flex items-center space-x-6">
                         <div className="text-center">
                             <div className="text-[10px] uppercase font-bold text-gray-400">Core Benchmarks</div>
                             <div className="text-lg font-bold text-unBlue">{getProgress('SUITE_A_CORE').toFixed(0)}%</div>
                         </div>
                         <div className="text-center">
                             <div className="text-[10px] uppercase font-bold text-gray-400">Adversarial Resilience</div>
                             <div className="text-lg font-bold text-orange-600">{getProgress('SUITE_B_ADVERSARIAL').toFixed(0)}%</div>
                         </div>
                         <button 
        ...
        <div className="flex justify-between pt-6 border-t border-gray-100">
            ...
            {step < 4 && (
                <button 
                    ...
                    className="px-6 py-2 rounded-lg bg-unDark text-white font-medium hover:bg-black flex items-center space-x-2"
                >
                    <span>Next Step</span>
                    <ArrowRight size={16} />
                </button>
